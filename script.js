...
class StudyPlanner {
    ...
    renderStudyPlan() {
        const listView = document.getElementById('studyPlan');
    ...
    toggleView() {
        ...
        toggleBtn.textContent = this.isCalendarView ? '목록 보기' : '달력 보기';
    }
    ...
    renderCalendarView() {
        ...
        // 시작 날짜와 종료 날짜 계산
        const startDate = new Date(document.getElementById('todayDate').value);
        const endDate = new Date(document.getElementById('examDate').value);
        
        // 달력을 시작 날짜의 월부터 종료 날짜의 월까지 생성
        ...
        while (currentDate <= lastMonth) {
            const monthCalendar = this.createMonthCalendar(currentDate, startDate, endDate);
            calendarView.appendChild(monthCalendar);
            currentDate.setMonth(currentDate.getMonth() + 1);
        }
    }

    createMonthCalendar(monthDate, studyStartDate, studyEndDate) {
        ...
        for (let i = firstDayOfWeek - 1; i >= 0; i--) {
            ...
            const dayElement = this.createCalendarDay(date, studyStartDate, studyEndDate, true);
        ...
        for (let date = 1; date <= lastDay.getDate(); date++) {
            ...
            const dayElement = this.createCalendarDay(currentDate, studyStartDate, studyEndDate, false);
            grid.appendChild(dayElement);
        }
        ...
        for (let date = 1; date <= remainingCells; date++) {
            ...
            const dayElement = this.createCalendarDay(nextMonthDate, studyStartDate, studyEndDate, true);
            grid.appendChild(dayElement);
        }
    ...
    createCalendarDay(date, studyStartDate, studyEndDate, isOtherMonth) {
        const dayElement = document.createElement('div');
        ...
        const dayPlan = this.studyPlan.find(plan => plan.date === dateString);
        
        if (dayPlan && date >= studyStartDate && date <= studyEndDate) {
        ...
        if (dayPlan) {
            ...
            subjectsContainer.className = 'calendar-subjects';
            
            dayPlan.subjects.forEach(subject => {
                const subjectTag = document.createElement('div');
                subjectTag.className = `calendar-subject-tag importance-${this.subjectImportance[subject]}`;
                subjectTag.textContent = subject;
                subjectTag.title = subject; // 툴팁으로 전체 이름 표시
    ...
    resetPlan() {
        this.studyPlan = [];
        this.isCalendarView = false;
        document.getElementById('toggleView').textContent = '달력 보기';
